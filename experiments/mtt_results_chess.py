...
track_path = "/nfs/chess/user/dbanco/SpotfetchMTT/singularity_system/tracker_states/"

for region_id in np.arange(5):
    try:
        # Load in tracker
        ...
        for scan in range(num_scans):
            print(f'Scan {scan}')
            # Load roi
            data = np.load(f'/nfs/chess/user/dbanco/SpotfetchMTT/singularity_system/region_files/region_{region_id}_scan_{scan}.npy')
            # Collect data series
            full_data[scan] = data
